-- ============================================================
-- ISO Policy Compliance App – Complete Supabase Schema
...
create table public.classification_history (
  ...
  prob_compliant      real not null default 0,
  prob_partial        real not null default 0,
  prob_non_compliant  real not null default 0,
...
create policy "Users can view own history"
  on public.classification_history for select using (auth.uid() = user_id);
create policy "Users can insert own history"
  on public.classification_history for insert with check (auth.uid() = user_id);
create policy "Users can delete own history"
  on public.classification_history for delete using (auth.uid() = user_id);

create index idx_history_user on public.classification_history(user_id, created_at desc);
...
create policy "Users CRUD own schedules"
...
create policy "Users CRUD own teams"
